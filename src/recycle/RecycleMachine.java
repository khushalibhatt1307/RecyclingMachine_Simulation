 ...
 * Class that represent RecycleMachine
 */
public class RecycleMachine {
    ...
    public RecycleMachine(int machineId,
                          String location,
                          ArrayList<ItemType> supportedItemTypes,
                          double capacity,
    ...
    public RecycleMachine(int machineId,
        ...
        this.totalBeginningMoney = totalBeginningMoney;
        if (isOperational)
            this.operationalStatus = OperationalStatus.ON;
        else
            this.operationalStatus = OperationalStatus.OFF;
    }
    ...
    */
    ...
    /*
     * method to check if item can be added or not in RecycleMachine
     */
    ...
    /*
     * method to add list of items in RCM
     */
    public double addItems(ArrayList<Item> itemsToAdd)
    {
        ...
        for (int j = 0; j < itemsToAdd.size(); j++)
        {
            ...
            totalDueAmount = totalDueAmount +
                    (itemsToAdd.get(j).getWeight() * itemsToAdd.get(j).getType().getPricePerLb() * itemsToAdd.get(j)
                            .getQuantity());
        }
